...
```
bandini.gestiondojo.ca.   A   103.98.215.181
```
...
Ajouter `bandini.gestiondojo.ca` a la liste des **hotes connus** du bloc
...
Voir `deploy/nginx/bandini-gestiondojo.conf.example` : c'est lui qui envoie
l'hote `bandini.gestiondojo.ca` vers le gunicorn du jeu (port 8006).
