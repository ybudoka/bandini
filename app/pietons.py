...
def _p(slug, nom, chandail, cheveux, peau, pantalon, *, vitesse=1.0, courage=0.0,
       temoin=0.3, vie=60, argent=(2, 20), arme=None, gang=None, frequence=1.0,
       phase=1) -> Pieton:
    return Pieton(
        slug=slug, nom=nom,
        couleurs={"c": chandail, "h": cheveux, "s": peau, "p": pantalon},
        vitesse=vitesse, courage=courage, temoin=temoin, vie=vie, argent=argent,
        arme=arme, gang=gang, frequence=frequence, phase=phase,
    )
...
def ordinaires() -> list[Pieton]:
    """Ceux qui peuplent la rue (les membres de gang apparaissent autrement)."""
