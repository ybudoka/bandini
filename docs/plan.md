...
|---|---|---|---|---|---|
| Le char abrite, l'appel fige | **en cours** | 15 sept. 2026 | **P2** | **correctif** | retour de Martin : « quand on est dans un voiture ou autre, il ne faut pas que les pietons puisse nous faire du domage » et « il faut aussi figer tout lors qu'on est au téléphone et qu'on ne peut pas bouger ». Deux promesses que le jeu ne tient pas, et la règle qui les répare est **déjà écrite une fois**, pour le feu : un brasier mord le CHAR et saute qui est dedans (`majBrasiers`, `c.dansVehicule`). Ni le poing ni la balle ne la connaissent — `arcDeMelee` et `majProjectiles` ne regardent que `vivant` et le type (`pieton` ou `joueur`) : un passant cogne à travers la portière, et une balle traverse la tôle comme si elle n'était pas là (⚠️ **rien** au monde n'endommage un char par balle aujourd'hui). Et `B.cinema` fige le JOUEUR (`majJoueur` lui met `vx = vy = 0`) sans figer la ville : au téléphone, on encaisse les coups, le trafic et la police sans pouvoir répondre — un menu, lui, fige déjà tout (`Jeu.maj`), et un fondu de porte aussi. En cours. |
...
| Le char tourne comme son ombre | **en cours** | 15 sept. 2026 | **P1** | **correctif** | retour de Martin : « le pilotage des véhicules est vraiment impossible maintenant, il faut que le véhicule tourne vraiment comme l'ombre le fait, sinon impossible de conduire ». Le char DEBOUT n'a que **quatre caps dessinés** (profil gauche, profil droite, dos, face) alors que son cap est **continu** : l'ombre pivote sous lui à chaque image, le dessin, lui, attend 45° et **claque** d'une pose à l'autre — et en claquant il SAUTE, parce que la ligne de sol du profil et celle du dos ne sont pas au même endroit (`solDeLaPose`, une demi-longueur d'écart). Au volant, on ne voit donc plus où on pointe : on lit le cap sur l'ombre. En cours de mesure — piste : le dessin **vu d'en haut** (`haut`/`bas`, qui porte déjà la longueur du char depuis la vue plongeante) est le seul des trois qui se laisse TOURNER, et c'est ce que faisait le parc d'avant (32 caps cuits par `Atlas.cuireRotations`). |
...
tests/  conftest.py harnais_js.py banc.js (bac à sable Node : faux canvas/DOM/fetch/manette/audio,
        ...
        test_reclame.py test_reclame_js.py test_argent_sale.py test_argent_sale_js.py test_contrebande.py test_contrebande_js.py test_barrieres.py test_barrieres_js.py test_ville_vit.py test_bagarre.py test_bagarre_js.py test_aqueduc.py test_aqueduc_js.py
