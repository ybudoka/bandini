...
| Un char qui ne reste plus pris dans un mur | **P1** **correctif**, **en cours** (13 sept. 2026) | demande de Martin (« mets un garde-fou pour éviter que mon véhicule coince dans un mur ou un objet »). ⚠️ `avancer` teste les tuiles **avant** chaque pas, mais rien ne regarde où le char **est** : poussé par un autre char (`heurterVehicules` déplace sans lire les tuiles), tourné sur place contre une façade (la chaîne de cercles pivote **dans** le mur) ou retombé d'un saut (en l'air, les tuiles ne comptent pas), il se retrouve dans le mur — et de là **chaque direction est bloquée** : pris pour toujours. Un **dégagement** à chaque image : pousser hors des tuiles chevauchées, sinon poser à la place libre la plus proche |
...
| La fille de la Brume parle | **P2** ajout, **en cours** (13 sept. 2026) | demande de Martin (« la prostituée aussi doit parler, avec plusieurs dialogues différents ») : `rumeurEtRepliques` saute tout piéton qui a un `metier`, et elle en a un (`compagnie`) — elle ne dit jamais rien, alors que la regex des voix de femmes la nomme déjà. Plusieurs répliques à elle (genre `brume`, une voix ElevenLabs), dites quand on passe près de son coin, avec une bulle |
...
tests/  conftest.py harnais_js.py banc.js (bac à sable Node : faux canvas/DOM/fetch/manette/audio,
        ...
        test_vehicules.py test_armes.py test_armes_js.py test_economie.py test_recherche.py test_carte.py
