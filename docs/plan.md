...
| Toute la musique générée par IA | **en cours** | 14 sept. 2026 | **P2** | **correctif** | demande de Martin : « je veux que toutes les musiques soient des musiques générées par IA ». Les **15 pièces écrites en notes** (thème du menu, 2 stations de char, 5 ambiances de district, poursuite, bagarre, 5 pièces du musicien de rue) deviennent des mp3 ElevenLabs Music — c'est la porte que `musique.py` annonce depuis le premier jour : « le jour où Martin veut une vraie pièce jouée par de vrais instruments, elle se posera **par-dessus** comme les radios ». La synthèse reste le **filet** : un fichier absent, et le séquenceur reprend |
| M11 La police apprend | **en cours** (3e vague) | 15 sept. 2026 | **P4** | ajout | ⚠️ **le casier PÈSE dans la rue** : il allonge la portée du cône des agents et des autos-patrouille (+4 % la page, **plafonné à +50 %** — sans ce plafond, vingt pages feraient voir la police à seize tuiles en pleine nuit et il n'y aurait plus une ruelle où souffler). ⚠️ **Pour le joueur seulement** : un casier épais n'aide pas la police à voir les passants — c'est un signalement, pas une paire de jumelles. Et le carnet du poste le **dit** (« ON TE RECONNAÎT : +X % DE LOIN ») : une règle qu'on subit sans jamais la lire n'est pas une règle, c'est une malchance. **2e vague livrée** : effacer une page, et c'est un **choix**, pas un bouton. **Me Desjardins**, table du fond du Brouillard : une page tout de suite, cher (1 200 $ + 400 $ la page au dossier), jamais deux fois le même jour — et l'autre moitié de sa fiche, une **provision** retenue d'avance qui efface l'amende de la prochaine arrestation. ⚠️ **Sans amende, pas innocent** : la page s'ajoute, les armes partent, le char va au lot — sinon se faire arrêter exprès devenait un trajet gratuit vers le poste. **Électronique Turcotte**, à La Shop : on paie d'avance, on revient le lendemain, et on ne sait pas ce qu'on a acheté — rien une fois sur trois, jusqu'à trois pages d'un coup, et **une page DE PLUS** une fois sur cinq. ⚠️ **Géographie tranchée (carte blanche de Martin)** : il ne fallait qu'**un seul lieu neuf**, pas deux — la table des personnages plaçait déjà `desjardins` au Brouillard, et un avocat qui tient salon au fond d'une taverne vaut mieux qu'une étude à lui. Seul le hacker prend un lot neuf (glyphe `E`), et il le prend à La Shop parce que c'est le **trajet** qui fait le risque : 0,15 la nuit. ⚠️ **Mon propre juge a attrapé mon trou** : à 200 $ la page de pente, le pari devenait la meilleure affaire au dollar à vingt pages, et l'avocat ne servait plus à rien pile quand on en a le plus besoin — pente passée à 260 $. **3e vague en cours** : le stool — ⚠️ **sans nouveau décor** : il se rue vers la porte d'un commerce pour téléphoner, et c'est la mécanique du flâneur qui rentre chez lui (`porteBut`), déjà écrite et déjà jugée. Une cabine aurait demandé un sprite et une règle de pose ; un gars qui rentre dans un dépanneur pour se servir du téléphone est aussi lisible et ne coûte rien. **Reste ensuite** : le bouclier humain. 9 juges neufs |
| Une passe visuelle sur les pâtés de maison | **en cours** | 15 sept. 2026 | **P2** | **correctif** | demande de Martin : « fais une passe visuelle d'amélioration de tous les pâtés de maison », puis « les affiches des commerçants doivent être au-dessus du mur » et « les clôtures doivent clôturer les terrains, pas juste être là seules ». Trois morceaux, un seul sujet : ce qu'on voit d'un îlot. ⚠️ **Le sol d'un îlot, c'est 43 % de la ville** (trottoir 28 %, herbe 10,5 %, ruelle 4,7 %) et il se peint en **quatre variantes** — quatre tuiles de 16 px répétées d'un bout à l'autre du Faubourg, dalle identique, joint identique : de loin, c'est du papier peint, et c'est la grille de la carte qu'on lit au lieu de la rue. ⚠️ **La ruelle n'est pas une ruelle** : une bande de deux tuiles derrière chaque bande d'îlot, gris plat, une poubelle par dix tuiles de large — le fond de cour de toute la ville est vide. ⚠️ **L'enseigne est écrasée dans le mur** : bandeau, auvent et vitre se partagent les 16 px de la tuile de façade, le nom tient sur cinq pixels de haut. ⚠️ **Une clôture ne clôture rien** : `poser_cloture` refuse en silence (tuile réservée, mur au nord) et il ne reste du U de `_jardin` que des bouts qui ne se rejoignent pas — trois palissades posées sur un gazon. |
| M10 L'argent sale | **à faire** (v2) | — | **P4** | ajout | le shylock et la dette de Rocco, guichets au camion, skimmers, assurance et fraude |
...
tests/  conftest.py harnais_js.py banc.js (bac à sable Node : faux canvas/DOM/fetch/manette/audio,
        ...
        test_interieurs_js.py test_rampes.py test_carte_du_depot.py test_eau.py test_banlieue.py test_parole.py test_effacer.py
...
| M11 | **P4** La police apprend | **3e vague, ce qui reste** : le stool, le bouclier humain | acheter le silence de celui qui t'a reconnu ; tenir un passant devant soi et voir la police baisser son arme |
