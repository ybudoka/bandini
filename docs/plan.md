...
| Toutes les façons de lancer ouvrent le réseau local | **en cours** | 15 sept. 2026 | **P3** | **correctif** | demande de Martin (« je veux que toutes les run soit accessible depuis mon reseau interne ») : `APP_HOST` vaut `127.0.0.1` par défaut, et **une seule** des cinq façons de lancer le serveur l'ouvre au wifi de la maison — la configuration VS Code « réseau local ». F5 « Jouer », la tâche `serveur`, `uv run python run.py` et Flask sans rechargement restent sourds au téléphone |
