...
|---|---|---|---|---|---|
| Un menu qui ne choisit rien | **en cours** | 15 sept. 2026 | **P2** | **correctif** | retour de Martin, capture à l'appui (« il n'y a pas de sélection dans ce menu ») : au comptoir des **hommes de Sal**, la ligne surlignée à l'ouverture est « LA DETTE » — un **en-tête** qu'on ne peut pas activer (`actif: false`, aucun `faire`), gris comme tout ce qui est hors de portée, et ACTION n'y répond qu'un bip. La règle existe pourtant : les OPTIONS la tiennent **à la main** (`curseur: 1`, leur première ligne étant un diagnostic) et un juge la garde. Mais chaque menu devait y penser tout seul, et **cinq l'ont oublié** — la dette, le carnet du poste, l'avocat, le comptoir du fond, la revente de contrebande : tous s'ouvrent sur une ligne qui se lit et ne se choisit pas. En cours. |
