...
| La première bagarre ne se gagne pas | **en cours** | 16 sept. 2026 | **P1** | **correctif** | retour de Martin : « la premiere mission des cravattes est trop difficile, ils font trop de dommage ». ⚠️ **Mesuré au banc, et il a raison** : M2 est la **première bagarre du jeu** — le joueur a 100 PV et ses **poings** (8 de dégâts), en face deux Cravates de **90 PV** armées du **bâton** (18 de dégâts, et `renverse`). Un joueur qui ne riposte pas tombe en **3 s** ; un joueur qui cogne sans rater une image met **4,3 s** à les coucher. La première bagarre exige donc un jeu **parfait**, et la dialogue promet le contraire (« Fais-leur comprendre. **Avec tes poings, pas plus.** »). ⚠️ Et le bâton est la **récompense** de M2 : on le rencontre avant de le posséder. |
