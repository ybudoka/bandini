...
| Le serveur ne redémarre plus pour un test | ⬜ **en cours** | 17 sept. 2026 | **P3** | **correctif** | demande de Martin : « il ne faut pas reloader l'app quand les tests ou autres dossier non impactant sont changé ». Sans `watchdog`, le rechargeur de Werkzeug est celui qui **parcourt tout `sys.path`** — donc toute la racine du dépôt : un `.py` modifié dans `tests/`, `scripts/` ou `deploy/` redémarre le serveur, et avec plusieurs sessions qui écrivent des juges en même temps, le jeu ouvert dans Chrome perd son serveur à chaque minute. |
...
tests/  conftest.py harnais_js.py banc.js (bac à sable Node : faux canvas/DOM/fetch/manette/audio,
        ...
        test_icones.py test_autobus.py test_autobus_js.py test_mobilier.py test_metro.py test_metro_js.py test_casque_js.py test_quartiers.py
