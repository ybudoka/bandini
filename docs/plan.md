...
|---|---|---|---|---|---|
| Un char garé dans ses lignes | **en cours** | 15 sept. 2026 | **P2** | **correctif** | retour de Martin, capture à l'appui (« les voitures sont mal garré ») : dans un stationnement, les chars débordent par le nez et laissent le fond de leur case vide. En cours de mesure — piste : la pose **de dos** porte la LONGUEUR du char depuis la vue plongeante (15 sept.), et elle est posée à la ligne de sol comme le profil : le dessin part donc du pare-chocs arrière et monte, pendant que l'empreinte, elle, est centrée sur `v.y`. |
...
tests/  conftest.py harnais_js.py banc.js (bac à sable Node : faux canvas/DOM/fetch/manette/audio,
        ...
        test_reclame.py test_reclame_js.py test_argent_sale.py test_argent_sale_js.py test_contrebande.py test_contrebande_js.py test_barrieres.py test_barrieres_js.py test_ville_vit.py test_bagarre.py test_bagarre_js.py
