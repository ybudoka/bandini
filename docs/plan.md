...
- ⚠️ **Mesuré : ils s'y arrêtent, à chaque feu rouge.** Depuis `TROTTOIR = 1`, la traverse
  fait UNE tuile, toute peinte, collée à la ligne d'arrêt — et le trafic attend **le
  centre** sur la ligne : le nez d'une berline mord 3–4 px dans les bandes, un camion 11–12,
  un autobus couvre la traverse entière. Au banc, 10 croisements × 2 000 images : **~150
  attentes sur un passage**, dont 120 à la ligne (feu, STOP, boîte) ; le reste : des chars
  pris DANS la boîte, et une panne posée contre une traverse. À faire : le nez à la ligne,
  pas le centre ; la panne loin des traverses ; un juge.
