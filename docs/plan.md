...
| L'Île-aux-Corneilles | ⬜ **en cours** | 16 sept. 2026 | **P4** | ajout | ⚠️ **Prête, et elle attend « La carte sort du paquet »** : avec elle, le paquet passe à **75 307 octets gzip** sur un plafond de 75 000 (la ville seule en pèse 74 472, l'île 835) — Martin (16 sept. 2026) : « carte hors du paquet d'abord ». demande de Martin : « tu peux extensionner la carte au besoin » — ⚠️ mesuré, le besoin est nul : **21 % de la carte est déjà de l'eau** (18 675 tuiles) et un rectangle de **40 × 24 tuiles d'eau pleine** attend au milieu de la baie. Une île, un quai, une chapelle, une usine à poisson fermée, **pas de police** (on y laisse refroidir un char et un casier), et une seule porte de sortie. Elle donne enfin une destination au traversier de M12 et à la fin _Le dernier traversier_ |
...
app/  __init__.py routes.py version.py scores.py definitions.py
      ...
      chantiers.py autobus.py mobilier.py metro.py salete.py
...
tests/  conftest.py harnais_js.py banc.js (bac à sable Node : faux canvas/DOM/fetch/manette/audio,
        ...
        test_rechargement.py test_icones.py test_autobus.py test_autobus_js.py test_mobilier.py test_metro.py test_metro_js.py test_casque_js.py test_quartiers.py
