...
| Le quai se marche, et la ville est moins sale | **en cours** | 16 sept. 2026 | **P1** | **correctif** | retour de Martin : « il y a **trop de saleté partout** et **impossible d'aller sur une partie du quai, il est clôturé, sans chemin à pied** ». ⚠️ **Mesuré, et les deux sont vrais — le second est P1 : une partie de la ville ne se visite pas.** **191 tuiles de quai sont inatteignables à pied**, en une quarantaine de poches. Et ce n'est PAS une clôture : c'est du décor solide. (1) **Deux semis posent des bornes d'amarrage sur la même lèvre** — `_meubler_le_quai` (écart 6) et `greve()` (`quai_poteau`, écart 3) — et la rangée du bord donne `QpQQAQQAQQQpQQQQQAQQpQQpQQAQQA` : une borne ou un pneu **tous les 3 pas sur soixante tuiles**. Vu d'en haut, c'est une palissade, et Martin l'a nommée comme telle. (2) **La cargaison couvre tout le tablier** : `PART_CARGAISON` sème une caisse par dix tuiles d'arrière, soit ~60 caisses SOLIDES sur dix rangées de profondeur — un plancher d'entrepôt, pas un quai. (3) **Les terrains vagues sont trop chargés** : un déchet par six tuiles, plus une mauvaise herbe par quatorze — 303 objets de saleté dans la ville (126 caisses, 65 sacs, 44 barils, 34 pneus, 34 gravats). Ce qui arrive : **un quai a un APRON** — la bande du bord reste dégagée (des bornes espacées, rien d'autre), la cargaison s'empile **au fond**, contre la rue par où les camions arrivent, et le milieu se marche ; la grève **cesse d'amarrer sur les planches** (le quai le fait chez lui) ; les densités baissent. Et une garantie plutôt qu'un réglage : **`carte.DECOR_SOLIDE`** dit en Python ce qui arrête un piéton, un juge de banc vérifie qu'elle dit la même chose que les fiches de dessin, et un juge tient qu'**aucun lot ni aucun quai ne se referme sur lui-même**. |
| Un hôpital qui soigne du monde | **en cours** | 16 sept. 2026 | **P4** | ajout | demande de Martin : « l'hôpital devrait être plus grand et avec des malades, une salle d'attente, des solutés, machines de santé ». ⚠️ **Mesuré** : la pièce fait **11 × 8** tuiles (9 × 6 de plancher, 54 tuiles), deux lits, trois chaises, un comptoir — et **pas un malade** : ses gens sont un commis et un client qui flâne. C'était la chambre de la planque avec du carrelage. Ce qui vient : une **urgence** au rez-de-chaussée (le triage, une salle d'attente où les patients sont **assis**, la distributrice du coin) et un **étage des soins** par l'escalier — des lits d'hôpital avec des **malades alités**, un **soluté** et un **moniteur** au chevet. Deux glyphes de meuble neufs et deux sortes de gens (`patient` assis, `malade` alité), jugés au chargement du module comme les autres. ⚠️ La pièce grandit, donc le **bâtiment** aussi : c'est `mesures_de_la_suite` qui taille sa parcelle, et les juges de la ville (enclaves, connexité) tournent sur plusieurs graines avant de conclure |
| Les machines distributrices | **en cours** | 16 sept. 2026 | **P4** | ajout | demande de Martin : « ajoute aussi un concept de machine distributrice partout dans la ville ». ⚠️ **Mesuré : zéro** dans la ville (1 665 décors de trente sortes). Le patron existe déjà, c'est le **guichet** : un décor encastré sous une devanture, sur l'abord, servi depuis la dalle, cassable, qui répand ce qu'il contient. La distributrice en est la petite sœur honnête : **trois sortes** (liqueur, grignotines, café) qui rendent ce que rendent déjà les kiosques (`nourrir`, `cafeine`) pour quelques piastres, **le long des façades** de toute la ville et **dans les salles d'attente** (l'urgence, le terminus). Défoncée, elle crache sa monnaie et ses canettes — un petit délit si quelqu'un regarde — et elle revient au matin comme le reste du décor. Ses chiffres vivent dans `economie.py`, et un juge tient qu'une nuit à vider les machines rapporte moins qu'une journée honnête |
