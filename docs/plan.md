...
| Trois sauvegardes, et on les gère | ⬜ **en cours** | 16 sept. 2026 | **P2** | ajout | demande de Martin : « on devrait pouvoir avoir 3 sauvegardes et les gérer ». Aujourd'hui une seule partie vit dans le `localStorage` (`bandini-partie-v1`) et le titre n'a qu'un bouton JOUER : recommencer, c'est effacer sa partie. Trois **emplacements** choisis au titre (jour, argent, temps de jeu, dernière fois), une partie neuve dans un emplacement vide, effacer (confirmé) et copier. ⚠️ La partie d'aujourd'hui devient l'emplacement 1 **sans bouger** : sa clé reste la même. |
...
tests/  conftest.py harnais_js.py banc.js (bac à sable Node : faux canvas/DOM/fetch/manette/audio,
        ...
        test_mise_en_scene.py test_scenes_js.py
