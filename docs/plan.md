...
| M16 Cent missions | ⬜ **en cours** (la tranche 1 « le moteur » avance : dix missions de plus livrées — `f04`, `f05`, `f06`, `f07`, `f09`, `f11`, `h01`, `p01`, `q03`, `e12` — avec le premier juge de banc joué pour huit des neuf types neufs ; **en cours : les dialogues et les scènes sortent du paquet** — `/api/dialogue/<slug>` avec son ETag, parce que le paquet est **au-dessus de son plafond** ; restent ensuite `eteindre` (pas de feu qu'une mission puisse allumer elle-même), le téléphone qui trie, et le reste de l'arc F) | 18 sept. 2026 | **P4** | ajout | [fiche](jalons/m16-cent-missions.md#fiche) · [notes](jalons/m16-cent-missions.md#notes) |
...
| Le scanner de police a perdu ses deux voix | ⬜ **à faire** (à trancher par Martin : le quota du mois est épuisé jusqu'au 17 oct.) | — | **P2** | **correctif** | [fiche](jalons/le-scanner-de-police-a-perdu-ses-deux-voix.md#fiche) |
...
| Les **vingt-quatre clips des répliques par contexte** (M15, 2e vague) | Le quota ElevenLabs du mois était à sec le 24 sept. 2026, jour de la livraison. Les textes, le jeu d'acteur et le moteur sont là ; `exporter()` ne déclare que les fichiers présents et une banque vide **se rabat sur `normal`** — la rue parle comme avant, elle ne se tait pas | **Le 17 oct. 2026**, quand le quota se remet : `uv run python scripts/audio_elevenlabs.py --voix` (elle ne génère que ce qui manque). ⚠️ Et **l'oreille de Martin** après : aucun juge ne dit qu'un son est le bon son |
