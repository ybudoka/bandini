...
| Les manettes du casque Meta Quest | ⬜ **en cours** | 17 sept. 2026 | **P4** | ajout | demande de Martin : « ajoute le support des manettes sur casque vr meta quest ». ⚠️ **Vérifié dans la doc Meta** (`webxr-pro-controller`) : les manettes Touch ne se lisent que par le **gamepad d'une session WebXR** (`XRInputSource.gamepad`, disposition `xr-standard`) — une page 2D ordinaire ne les voit pas dans `navigator.getGamepads()`. Il faut donc un **mode casque** : un bouton JOUER DANS LE CASQUE (le clic de la gâchette sur la page est un vrai geste, le son suit), le jeu projeté sur un écran virtuel, et les deux manettes lues comme une manette Xbox (A action, B course/retour, X frappe, Y arme, poignées = épaules, gâchettes = gaz/frein, clic des sticks = pause/carte) |
...
tests/  conftest.py harnais_js.py banc.js (bac à sable Node : faux canvas/DOM/fetch/manette/audio,
        ...
        test_icones.py test_autobus.py test_autobus_js.py test_mobilier.py test_metro.py test_metro_js.py test_quartiers.py
