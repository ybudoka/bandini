...
| La carte sort du paquet | **en cours** | 16 sept. 2026 | **P3** | ajout | décision de Martin (16 sept. 2026), devant l'Île-aux-Corneilles qui faisait passer le paquet à 75,3 Ko gzip sur 75 : **la carte sort du paquet** plutôt que de relever le plafond une fois de plus — c'est ce que `test_definitions` écrivait d'avance (« c'est à ce plafond-ci qu'on le prendra »). `/api/carte` avec son propre ETag, les deux requêtes au démarrage, et le navigateur remet la carte dans `B.defs` : aucun lecteur de la carte ne change. ⚠️ **La sauvegarde oublie une position quand la CARTE change** (l'empreinte) : elle doit suivre la carte, pas le paquet. ⚠️ Les districts chargés autour du joueur restent une dette (« Dettes »). |
...
| `routes.py` | `/`, `/api/definitions` (ETag, 304), `/api/scores` GET/POST, `/sante`, `/manifest.webmanifest`, `/favicon.ico`, 404 « Cul-de-sac » | page, ETag/304, scores, 413 |
