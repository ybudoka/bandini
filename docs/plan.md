...
| Le port du Faubourg touche l'eau, et les chaloupes mouillent dans la baie | **en cours** | 16 sept. 2026 | **P2** | **correctif** | retour de Martin, capture à l'appui : « il y a encore des quais entre deux routes ». ⚠️ **Mesuré** : les deux quais du Faubourg (`qq` au plan) sont du plancher plein **entouré de rues** — une rue de chaque côté, et dessous un boulevard à quatre voies, une plage, puis la baie. Le défaut des Quais, un bloc plus à l'est, et il était nommé comme « reste ouvert » dans la livraison du port. ⚠️ **Et un second, trouvé en mesurant** : **8 des 18 chaloupes ne mouillent pas dans la baie** — trois dans l'étang du Faubourg, trois dans le chenal de La Pointe, deux dans des **mares de parc de 2 et 6 tuiles**. Le semis parcourt la carte du nord au sud et s'arrête à dix-huit : il remplit les mares du nord avant d'atteindre le port. |
| Une chaloupe sur la route | **en cours** | 16 sept. 2026 | **P2** | **correctif** | retour de Martin, capture à l'appui : « un bateau sur la route ?? » — une chaloupe arrêtée dans sa voie au passage piéton, cap à l'est. ⚠️ **Lu dans le code** : le voleur de char (`Entites.majVolDeChar`) prend le premier véhicule garé à l'écran, et une coque amarrée est `stationne` comme une auto ; volée, elle passe au trafic (`conducteur: 'trafic'`), et **le trafic roule sur des rails sans lire une seule tuile** (`v.x += v.vx`) — `tuileInterdite`, qui tient la coque sur l'eau, n'est jamais consultée. La voie la plus proche (`voieLaPlusProche`, trois tuiles) est sur le quai : elle y monte et fait sa tournée. |
| Le motard revient sur la moto volée | **en cours** | 16 sept. 2026 | **P1** | **correctif** | retour de Martin : « quand on vole une moto, la personne qui était dessus s'en va, mais quand on la quitte, il y a encore une personne dessus ». ⚠️ **Lu dans le code** : `Vehicules.monter` efface le pilote du trafic (`v.pilote = null`) dans la seule branche du **vélo** ; la moto passe par celle du carjacking, qui fait sortir un passant tiré au hasard et laisse `v.pilote` en place. Au volant, `cavalierDe` peint le joueur ; descendu, il retombe sur l'ancien motard. |
