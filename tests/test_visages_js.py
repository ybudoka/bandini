...
def test_la_replique_montre_le_visage_de_qui_parle_avec_sa_mine(banc, paquet, a_jouer):
    """La première réplique de Ti-Guy : son portrait, à gauche, et le texte poussé à droite."""
    humeur = next((r.get("humeur") for r in a_jouer["m1"]["dialogue"]["intro"] if r["qui"] == "ti_guy"), None)
    r = banc("""function (L, o) {
        L.Jeu.commencer();
