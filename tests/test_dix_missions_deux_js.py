...
def test_p14_un_seul_bonimenteur_qui_attend_puis_court_sur_nos_pas_jusqu_au_poste(banc):
    ...
    r = banc("function (L, o) {" + OUTILS + ESCORTE + """
        L.Jeu.commencer(); L.graine(6);
        const B = L.B, j = B.joueur; j.invincible = 1e6;
        faites(L, ['m1', 'm2', 'm3', 'm4', 'm5', 'm6', 'p13']);
        ...
        for (let k = 0; k < 120 && etape(L) === 0; k++) { o.frame(1); fermer(L); }
    ...
    assert r["attend"] == {"bouge": 0, "suit": False, "gpsSurLui": True, "seulAuPoste": 0}, (
    ...
    assert r["etape"] == 1 and r["luiAuPoste"] < 7 * 16, f"arrivés ENSEMBLE au poste, l'objectif avance : {r}"
...
def test_p14_il_monte_dans_le_char_descend_au_poste_puis_rentre_a_l_arche(banc):
    ...
    r = banc("function (L, o) {" + OUTILS + ESCORTE + """
        ...
        for (let k = 0; k < 10 && etape(L) === 0; k++) o.frame(1);
        ...
        fermer(L);
        const skateux = B.mission.entites.filter(function (e) { return e.cible && e.etape === 1; });
    ...
    assert r["arrive"]["etape"] == 1 and r["arrive"]["dehors"] and r["arrive"]["pres"] < 40, (
...
def test_p14_protege_assomme_la_mission_echoue_et_il_rentre_a_l_arche(banc):
    ...
    assert r["seul"] == {"etape": 0, "suit": True}, f"rejoint, mais seul au poste : l'objectif attend qu'il y soit aussi : {r}"
